...
##########################
library(readr)
LPS24 <- read_delim("LPS/LPS24.47231.322.b.txt", 
                   "\t", escape_double = FALSE, trim_ws = TRUE)
miss_pheno <- setdiff(seq(1, 432), as.numeric(colnames(LPS24)[2:323])) #samples missing in phenotype
remov_geno <- setdiff(miss_pheno, c(14, 82, 111, 165, 215, 291, 320, 375, 386, 389)) #samples to be removed from genotype

#write .txt for PLINK
write.table(x = cbind(remov_geno, remov_geno), file = "remove_pheno_LPS24.txt", quote = FALSE,sep = "\t", row.names = FALSE, col.names = FALSE )

#From phenotype we only need to remove the sample 111 (column 49)
LPS24_clean <- subset(LPS24, select = -c(49)) 
dim(LPS24_clean) #320 SAMPLES
...
for (probe in LPS24_clean$PROBE_ID){
...
llista_SNP <- intersect(LPS24_clean$PROBE_ID, SNP) #14.899 probes with SNPs
...
llista_no_match <- intersect(No_match, LPS24_clean$PROBE_ID) #376 probes with No match

Bad <- colnames(data.frame(xx[grep("Bad", xx)])) 

llista_bad <- intersect(Bad, LPS24_clean$PROBE_ID) #12.379 probes with Bad


################################# Disccard probes ####################################
LPS24_clean <- subset(LPS24_clean, !(PROBE_ID %in% list_positions)) #remove probes +1 locus
dim(LPS24_clean) #44008 probes

LPS24_clean <- subset(LPS24_clean, !(PROBE_ID %in% list_non_chr))
dim(LPS24_clean) #43658 probes

LPS24_clean <- subset(LPS24_clean, !(PROBE_ID %in% list_sex))
dim(LPS24_clean) #41489 probes

LPS24_clean <- subset(LPS24_clean, !(PROBE_ID %in% llista_SNP))
dim(LPS24_clean) #28029 probes

LPS24_clean <- subset(LPS24_clean, !(PROBE_ID %in% llista_no_match))
dim(LPS24_clean) #28008 probes

LPS24_clean <- subset(LPS24_clean, !(PROBE_ID %in% llista_bad))
dim(LPS24_clean) #20808 probes 
...
blast_seq <- read_delim("blast_seq.txt", 
                        ...
                        trim_ws = TRUE, )
setdiff(blast_seq$X1, LPS24_clean$PROBE_ID)
...
LPS24_clean <- subset(LPS24_clean, !(PROBE_ID %in% cross_hyb$X1))
dim(LPS24_clean) #14512 final probes 
...
rownames(df_positions) <- LPS24_clean$PROBE_ID

head(df_positions)


final_df <- cbind(df_positions, LPS24_clean)
...
write.table(x = final_df, file = "phenotype_LPS24.txt", sep = "\t", quote = FALSE, row.names = FALSE)
